...
Model: YOLOv12s-seg
Cấu trúc: Calibration (10s) → Lock → Detection → Export Video
Phiên bản: Voting + Convex Hull + Multi-Point Check (points shifted UP) + Video Output
"""
...
OUTPUT_VIDEO_PATH = "output_sidewalk_violation.avi"  # .avi thường ổn định hơn .mp4
...
def main():
    ...
    global out
    global OUTPUT_VIDEO_PATH  # <-- THÊM DÒNG NÀY ĐỂ TRÁNH LỖI UnboundLocalError
    ...
    if out is not None:
        out.release()
        print(f"\n✅ Video output đã được lưu tại: {os.path.abspath(OUTPUT_VIDEO_PATH)}")
